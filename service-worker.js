...
This is generally NOT safe. Learn more at https://bit.ly/wb-precache`;
    ...
    */
      ...
      if (!response) {
        try {
          const networkResponse = await fetch(
            `${CONFIG.API_BASE}${url}`.replace(/([^:])(\/\/+)/g, "$1/")
          );
    ...
    function isOfflineError(error) {
      return error.name === "TypeError" && error.message.includes("Failed to fetch") && !window.navigator.onLine;
    }

    class LeagueRepository {
      static async findAll() {
        const apiLeagues = await fetchApi(
          `/api/leagues.json`,
          () => []
        );
      ...
      static async findById(id) {
        const league = (await this.findAll()).find(
          (league2) => league2.id === id
        );
        if (!league) {
    ...
    class FieldRepository {
      static FIELD_CACHE = {};
      static async findAll() {
        const apiFields = await fetchApi(
          `/api/fields.json`,
          () => []
        );
      ...
      static async findByKeyword(keyword) {
        const cachedField = FieldRepository.FIELD_CACHE[keyword];
        if (cachedField) {
          return cachedField;
        }
        if (keyword.trim() === ",") {
    ...
    class TeamRepository {
      ...
      static async findByName(name) {
        ...
        for (const team of teams) {
          for (const [mappingName, mappingId] of Object.entries(
            TeamRepository.TEAM_MAPPING
          )) {
    ...
    self.addEventListener("install", async (event) => {
      event.waitUntil(
        (async () => {
          const jsonCache = await caches.open("api-json-cache-v1");
          const jsonUrls = await getPrecachedJsonApiUrls();
          await jsonCache.addAll(jsonUrls);
          const imageCache = await caches.open("api-assets-cache-v1");
          const assetUrls = await getPrecachedAssetUrls();
          await imageCache.addAll(assetUrls);
        })()
      );
    ...
    registerRoute(
      new RegExp(
        `^${escapeRegExp(CONFIG.API_BASE)}assets/.*\\.(?:png|jpg|jpeg|svg|webp)$`
      ),
      new CacheFirst({
    ...
    self.addEventListener("message", async (event) => {
      if (event.data && event.data.type === "triggerPushEvent") {
        const now = /* @__PURE__ */ new Date();
        const state = getState();
        const games = [];
        for (const league of state.leagues) {
          games.push(
            ...await GamesRepository.findGamesForDay(
              now,
              CONFIG.SEASON,
              league
            )
